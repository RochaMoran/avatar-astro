// src/scripts/avatarLoader.js
import * as THREE from "three";
import { GLTFLoader } from "three/addons/loaders/GLTFLoader.js";
import { OrbitControls } from "three/addons/controls/OrbitControls.js";

export function loadAvatar() {
  // JavaScript to load 3D Avatar
  window.onload = () => {
    const loader = new GLTFLoader();
    loader.load(
      "src/models/model-2.glb",
      function (gltf) {
        console.log(gltf);
        setupScene(gltf);
  ...
  function setupScene(gltf) {
    ...
    controls.enableZoom = true; // Puedes cambiar esto a `true` si deseas permitir el zoom manual con el mouse
    controls.minDistance = 1; // Reduce este valor para permitir que la cámara se acerque más
    controls.maxDistance = 5; // Define un límite para alejar la cámara si es necesario
    ...
    const stumbleClip = THREE.AnimationClip.findByName(
      clips,
      "Armature_3|mixamo.com|Layer0"
    // ""
    );
    ...
    container.addEventListener("mousedown", (ev) => {
      ...
      if (intersections.length > 0) {
        ...
        stumbleAction.loop = THREE.LoopOnce;
        // stumbleAction.timeScale = 0.5;
        // stumbleAction.time = 0;
